...
class log extends JFrame implements ActionListener 
{
	...
	log() 
	{
		...
		setTitle("Student or Parent Login");
		...
		if(ae.getSource()==log)
		{
			try
			{
				...
				stmt=conn.createStatement();
		         	rs=stmt.executeQuery("select student_id from student");
		         	stmt1=conn.createStatement();
		         	rs1=stmt1.executeQuery("select student_password from LoginInfo");
	
			     while(rs.next()&&rs1.next())
			     {
			    	 
			    	 if(u==rs.getInt(1) && p.equals(rs1.getString(1)))
			    	 {
			    		dispose();
			    		flag=true;
			    		new dashboard();
